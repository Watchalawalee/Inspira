...
class AllThaiEventSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if event_count == 0:
            no_data_months += 1
            self.log(f"🛑 ไม่พบอีเว้นท์ในลิงก์: {response.url} — ข้ามเดือนนี้ (เดือนที่ไม่มีข้อมูล: {no_data_months})")

            if no_data_months >= 6:
                self.log("🚫 ไม่พบข้อมูลติดต่อกันครบ 6 เดือน — หยุดทำงาน")
                return
        else:
            no_data_months = 0  # รีเซ็ตถ้าเดือนนี้มีข้อมูล
        ...
        yield scrapy.Request(next_url, callback=self.parse, meta={"no_data_months": no_data_months})
