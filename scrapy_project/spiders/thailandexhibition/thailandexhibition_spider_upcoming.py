...
class EventSpider(scrapy.Spider):
    name = 'thailandexhibition_spider_upcoming'
    ...
    def parse_event_details(self, response):
        try:
            ...
            # ✅ กรองเฉพาะงานที่เริ่มตั้งแต่ มีนาคม 2025 เป็นต้นไป และมีสถานะ ongoing หรือ upcoming เท่านั้น
            start_obj = dateparser.parse(start_date, languages=["th", "en"])
            march_2025 = datetime.date(2025, 3, 1)

            if not start_obj or start_obj.date() < march_2025 or status not in ["upcoming", "ongoing"]:
                return
